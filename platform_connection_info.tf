...
#
# Sibling Terraform stacks (e.g. ipsupport-llc/lineoneagent-infra
# under GCP) and other tooling that needs to plug into this
# platform read these coordinates to discover IdP, secrets-store,
# ingress base, and CI runner labels — without grepping
# `config/platform.yaml` or copy-pasting hostnames.
#
# Tiering (see handoff letter
# `~/agent-inbox/lineoneagent-infra/inbox/2026-05-23-platform-arc-runner-ideas-and-gcs-bootstrap-runbook.md`):
#
...
output "platform_connection_info" {
  ...
  value = {
    ...
    # Primary mail domain (e.g. `ipsupport.us`) — convenient for
    # consumers that want to derive `admin@${primary_domain}` or
    # similar without re-reading domain yaml. Empty when no mail
    # domain configured.
