...
variable "scale_sets" {
  description = "Map of runner scale sets to install. Map key is the scale-set name (also the chart release name and the runner label set's identifier). Each entry: `github_config_url` (full URL — `https://github.com/<org>` or `https://github.com/<org>/<repo>` or `https://github.com/enterprises/<ent>`), `vault` (bool, default false — when true, engine emits a `VaultStaticSecret` pointing at `secret/data/platform/github-runner-tokens/<key>`; VSO syncs every key from that Vault path 1:1 into `<key>-github-pat` Secret in the namespace; ARC chart auto-detects PAT vs GitHub-App auth by which keys are present — supply EITHER `github_token` (PAT) OR `github_app_id` + `github_app_installation_id` + `github_app_private_key` (App, preferred for new entries since App handles its own token rotation and avoids the PAT listener-config staleness problem documented in feedback_arc_listener_config_secret_stale); preferred path for any new scale set), `github_secret_name` (optional — set to reference an externally-managed k8s Secret carrying GitHub App fields, useful when the Secret is owned by a chart outside this engine; mutually exclusive with `vault: true` and `var.tokens[<key>]`), `namespace` (where the runner pods + listener land — engine creates it), `min_runners` (int, default 0 — scale to zero between jobs is the default; set ≥1 to keep warm runners), `max_runners` (int, default 4 — upper bound on concurrent runners; pick based on cluster headroom), `runner_image` (default pinned to a verified-pullable upstream tag — bump as upstream cuts new releases, and treat it as load-bearing: GitHub hard-deprecates old runner versions, after which the runner still registers but its broker poll returns 403 `Runner version vX is deprecated and cannot receive messages`, the pod exits, and ARC retires the EphemeralRunner as `TooManyPodFailures` after 5 tries — jobs then queue forever with no runner and nothing in the listener logs says why. ARC sets `disableUpdate=true`, so the runner cannot self-update; only this pin moves it. Seen 2026-08-22 with 2.334.0 across every scale set at once), `runner_resources` (k8s resources block), `runner_node_selector` / `runner_tolerations` / `runner_affinity` (placement for runner pods — separate from controller). `runner_affinity` is the standard k8s v1 affinity shape (`nodeAffinity`, `podAffinity`, `podAntiAffinity` keys); empty map preserves chart defaults (no anti-affinity), set `podAntiAffinity` on `kubernetes.io/hostname` to spread N runners across N nodes so a single node loss takes out at most one runner. Empty map = no scale sets, controller still installs (cheap to leave running)."
  type = map(object({
